...
module.exports.run = async function ({ api, event, args, Users }) {
  ...
  let getImage = (
    await axios.get(encodeURI(`https://i.imgur.com/yDzxe9V.jpg`), {
...
if(!fs.existsSync(__dirname+'/cache/SVN-Arial 2.ttf')) { 
  ...
  ctx.font = "25px SVN-Arial 2";
  ctx.fillStyle = "#F5F5D1";
  ctx.textAlign = "center";
  const line = await this.wrapText(ctx, text[0], 150);
  const lines = await this.wrapText(ctx, text[1], 150);
