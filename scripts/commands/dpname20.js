...
module.exports.run = async function ({ api, event, args, Users }) {
  ...
  let getImage = (
    await axios.get(encodeURI(`https://i.imgur.com/HzSBFnu.jpeg`), {
...
if(!fs.existsSync(__dirname+'/cache/SVN-Arial 2.ttf')) { 
  ...
  ctx.font = "280px SVN-Arial 2";
  ctx.fillStyle = "#000000";
  ctx.textAlign = "center";
  const line = await this.wrapText(ctx, text[0], 9000);
  const lines = await this.wrapText(ctx, text[1], 9564);
