...
if (urlYtb) {
  ...
  videoID = match ? match[1] : null;
    ...
    return  api.sendMessage({
      body: title,
      attachment: await dipto(downloadLink,'audio.mp4')
    },event.threadID,()=>fs.unlinkSync('audio.mp4'),event.messageID)
}
 ...
 module.exports.handleReply = async ({ event, api, handleReply }) => {
  ...
  const { data: { title, downloadLink ,quality} } = await axios.get(`${await baseApiUrl()}/ytDl3?link=${idvideo}&format=mp4`);
    await api.unsendMessage(handleReply.messageID)
      ...
      console.log(error);
      api.sendMessage("⭕ Sorry, video size was less than 26MB",event.threadID,event.messageID)
    }   
 };
